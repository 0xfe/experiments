...
Mandelbrot.prototype.PlotCount = function(x, y, count, signal) {
  if (count < 10) {
    this.Plot(x, y, rgb(0, 0, count * 10));
  } else if (count < 40) {
    this.Plot(x, y, rgb(0, 0, count * 5));
  } else {
    this.Plot(x, y, rgb(0, count, count * 2));
  }
}
...
Mandelbrot.prototype.ProgressiveDraw = function(xcorner, ycorner, zoom,
  ...
  for (var x = xstart; x < xend; x++) {
    for (var y = 1; y < this.height; y++) {
      ...
      var signal = (count - logBase(logBase(Math.sqrt(size), 2), 2));
      if (y == 200 && count < 10) console.log(size, signal);
      this.PlotCount(x, y, count, signal);
    }
  }
}
